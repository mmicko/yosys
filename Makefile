...
BINDIR := $(PREFIX)/bin
LIBDIR := $(PREFIX)/lib
DATDIR := $(PREFIX)/share/$(BUILD_PREFIX)yosys
...
EXTRA_OBJS =
EXTRA_TARGETS =
TARGETS = $(BUILD_PREFIX)yosys$(EXE) $(BUILD_PREFIX)yosys-config
...
TARGETS := $(filter-out $(BUILD_PREFIX)yosys-config,$(TARGETS))
...
TARGETS += lib$(BUILD_PREFIX)yosys.so
endif
...
ifeq ($(ABCEXTERNAL),)
TARGETS += $(BUILD_PREFIX)yosys-abc$(EXE)
endif
endif
endif
...
P_STATUS = 0
P_OFFSET = 0
P_UPDATE = $(eval P_STATUS=$(shell echo $(OBJS) $(BUILD_PREFIX)yosys$(EXE) | $(AWK) 'BEGIN { RS = " "; I = $(P_STATUS)+0; } $$1 == "$@" && NR > I { I = NR; } END { print I; }'))
P_SHOW = [$(shell $(AWK) "BEGIN { N=$(words $(OBJS) $(BUILD_PREFIX)yosys$(EXE)); printf \"%3d\", $(P_OFFSET)+90*$(P_STATUS)/N; exit; }")%]
...
kernel/yosys.o: CXXFLAGS += -DYOSYS_DATDIR='"$(DATDIR)"' -DYOSYS_BUILD_PREFIX='"$(BUILD_PREFIX)"'
...
ifeq ($(LINK_ABC),1)
OBJS += $(BUILD_PREFIX)yosys-libabc.a
endif
...
$(BUILD_PREFIX)yosys$(EXE): $(OBJS)
	$(P) $(LD) -o $(BUILD_PREFIX)yosys$(EXE) $(LDFLAGS) $(OBJS) $(LDLIBS)

lib$(BUILD_PREFIX)yosys.so: $(filter-out kernel/driver.o,$(OBJS))
ifeq ($(OS), Darwin)
	$(P) $(LD) -o lib$(BUILD_PREFIX)yosys.so -shared -Wl,-install_name,lib$(BUILD_PREFIX)yosys.so $(LDFLAGS) $^ $(LDLIBS)
else
	$(P) $(LD) -o lib$(BUILD_PREFIX)yosys.so -shared -Wl,-soname,lib$(BUILD_PREFIX)yosys.so $(LDFLAGS) $^ $(LDLIBS)
endif
...
$(BUILD_PREFIX)yosys-config: misc/yosys-config.in
	$(P) $(SED) -e 's#@CXXFLAGS@#$(subst -I. -I"$(YOSYS_SRC)",-I"$(DATDIR)/include",$(strip $(CXXFLAGS_NOVERIFIC)))#;' \
			...
			-e 's#@BINDIR@#$(strip $(BINDIR))#;' -e 's#@DATDIR@#$(strip $(DATDIR))#;' < $< > $(BUILD_PREFIX)yosys-config
	$(Q) chmod +x $(BUILD_PREFIX)yosys-config
...
$(BUILD_PREFIX)yosys-abc$(EXE): abc/abc-$(ABCREV)$(EXE)
	$(P) cp abc/abc-$(ABCREV)$(EXE) $(BUILD_PREFIX)yosys-abc$(EXE)

$(BUILD_PREFIX)yosys-libabc.a: abc/libabc-$(ABCREV).a
	$(P) cp abc/libabc-$(ABCREV).a $(BUILD_PREFIX)yosys-libabc.a
...
# Unit test
unit-test: lib$(BUILD_PREFIX)yosys.so
...
install: $(TARGETS) $(EXTRA_TARGETS)
	...
	$(INSTALL_SUDO) cp $(filter-out lib$(BUILD_PREFIX)yosys.so,$(TARGETS)) $(DESTDIR)$(BINDIR)
ifneq ($(filter $(BUILD_PREFIX)yosys,$(TARGETS)),)
	$(INSTALL_SUDO) $(STRIP) -S $(DESTDIR)$(BINDIR)/$(BUILD_PREFIX)yosys
endif
ifneq ($(filter $(BUILD_PREFIX)yosys-abc,$(TARGETS)),)
	$(INSTALL_SUDO) $(STRIP) $(DESTDIR)$(BINDIR)/$(BUILD_PREFIX)yosys-abc
endif
ifneq ($(filter $(BUILD_PREFIX)yosys-filterlib,$(TARGETS)),)
	$(INSTALL_SUDO) $(STRIP) $(DESTDIR)$(BINDIR)/$(BUILD_PREFIX)yosys-filterlib
endif
	$(INSTALL_SUDO) mkdir -p $(DESTDIR)$(DATDIR)
	$(INSTALL_SUDO) cp -r share/. $(DESTDIR)$(DATDIR)/.
ifeq ($(ENABLE_LIBYOSYS),1)
	$(INSTALL_SUDO) mkdir -p $(DESTDIR)$(LIBDIR)
	$(INSTALL_SUDO) cp lib$(BUILD_PREFIX)yosys.so $(DESTDIR)$(LIBDIR)/
	$(INSTALL_SUDO) $(STRIP) -S $(DESTDIR)$(LIBDIR)/lib$(BUILD_PREFIX)yosys.so
ifeq ($(ENABLE_PYOSYS),1)
	$(INSTALL_SUDO) mkdir -p $(PYTHON_DESTDIR)/$(subst -,_,$(BUILD_PREFIX))pyosys
	$(INSTALL_SUDO) cp lib$(BUILD_PREFIX)yosys.so $(PYTHON_DESTDIR)/$(subst -,_,$(BUILD_PREFIX))pyosys/libyosys.so
	$(INSTALL_SUDO) cp misc/__init__.py $(PYTHON_DESTDIR)/$(subst -,_,$(BUILD_PREFIX))pyosys/
endif
endif
...
ifeq ($(ENABLE_LIBYOSYS),1)
	$(INSTALL_SUDO) rm -vf $(DESTDIR)$(LIBDIR)/lib$(BUILD_PREFIX)yosys.so
ifeq ($(ENABLE_PYOSYS),1)
	$(INSTALL_SUDO) rm -vf $(PYTHON_DESTDIR)/$(subst -,_,$(BUILD_PREFIX))pyosys/libyosys.so
	$(INSTALL_SUDO) rm -vf $(PYTHON_DESTDIR)/$(subst -,_,$(BUILD_PREFIX))pyosys/__init__.py
	$(INSTALL_SUDO) rmdir $(PYTHON_DESTDIR)/$(subst -,_,$(BUILD_PREFIX))pyosys
endif
endif

update-manual: $(TARGETS) $(EXTRA_TARGETS)
	cd manual && ../$(BUILD_PREFIX)yosys -p 'help -write-tex-command-reference-manual'
...
clean-abc:
	...
	rm -f $(BUILD_PREFIX)yosys-abc$(EXE) $(BUILD_PREFIX)yosys-libabc.a abc/abc-[0-9a-f]* abc/libabc-[0-9a-f]*.a

mrproper: clean
	git clean -xdf

coverage:
	./$(BUILD_PREFIX)yosys -qp 'help; help -all'
...
mxebin: $(TARGETS) $(EXTRA_TARGETS)
	...
	cp -r $(BUILD_PREFIX)yosys.exe share/ yosys-win32-mxebin-$(YOSYS_VER)/
ifeq ($(ENABLE_ABC),1)
	cp -r $(BUILD_PREFIX)yosys-abc.exe abc/lib/x86/pthreadVC2.dll yosys-win32-mxebin-$(YOSYS_VER)/
endif
